    ...
    stages {
        ...
        stage('DeployToProduction') {
            ...
            steps {
                ...
                milestone(1)
                withCredentials([usernamePassword(credentialsId: 'webserver_login', usernameVariables: 'USERNAMME', passwordVariable 'USERPASS')]) {
                    script {
                        sh "sshpass -p 'USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@prod_id \"docker pull driver23/train-schedule:${env.BUILD_NUMBER}\""
                        try {
